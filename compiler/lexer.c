...
// bi-direction ensures each character is correct
...
static char nextc()
{
    ...
    // (40 + 30) , store 40+30 to the parentheses_buffer, then assign to between_brackets
...
{
    /* these operators can't be merged such as ((, [[, ** */
...
{
    // if concat operator is invalid such as +*, -*...
    // then push back the following operator
...
{
    // once '(' is detected, it means there is a new expression
...
struct token *token_make_multi_line_comment()
{
    ...
    while (1)
    {
        ...
        if (c == EOF)
        {
            compiler_error(lex_process->compiler, "You didn't close the multi-line comment");
        }
        else if (c == '*')
        {
            /* skip the * */
...
void lexer_validate_binary_string(const char *str)
{
    ...
    for (int i = 0; i < len; i++)
    {
        if (str[i] != '0' && str[i] != '1')
        {
            compiler_error(lex_process->compiler, "This is not a validate binary number\n");
        }
    }
}
...
struct token *token_make_special_number_binary()
{
    ...
    nextc();
    unsigned long number = 0;
    const char *number_string = read_number_str();
    lexer_validate_binary_string(number_string);
...
struct token *token_make_quote()
{
    ...
    if (nextc() != '\'')
    {
        compiler_error(lex_process->compiler, "You opened a quote ' but didn't close it whit a character '");
    }

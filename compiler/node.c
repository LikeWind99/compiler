...
{
    // safe pop node, when pop it will check the node_vector is empty or not
    struct node *last_node = vector_back_ptr(node_vector);
    struct node *last_node_root = vector_empty(node_vector) ? NULL : vector_back_ptr(node_vector);
...
#warning "we shoule set the binded owner and binded function here"
    node_push(node);
    return node;
}